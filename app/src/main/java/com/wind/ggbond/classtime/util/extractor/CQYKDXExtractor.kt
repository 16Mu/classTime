...
@Singleton
class CQYKDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    override val schoolId = "cqykdx"
    override val schoolName = "重庆医科大学"
    override val systemType = "shuwei"

    override val aliases = listOf("重庆医科大学", "重医")
    override val supportedUrls = listOf("jwc.cqmu.edu.cn")
    companion object { private const val TAG = "CQYKDXExtractor" }
    ...
    override fun getLoginUrl() = "http://jwc.cqmu.edu.cn/"
    override fun getScheduleUrl() = "http://jwc.cqmu.edu.cn/eams/courseTableForStd.action"
    
    override fun generateExtractionScript() = """
        (function() {
            try {
                console.log('🔍 开始提取重庆医科大学课表（树维系统-版本一）...');
                var courses = [];

...
@Singleton
class LDXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "ldxy"
    override val schoolName = "陇东学院"
    override val systemType = "kingosoft"

    override val aliases = listOf("陇东学院", "陇东")
    override val supportedUrls = listOf("jwgl.ldxy.edu.cn")
    
    companion object {
        private const val TAG = "LDXYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("ldxy.edu.cn") && (url.contains("xskb") || html.contains("frmDesk"))
    }
    
    override fun getLoginUrl(): String = "http://jwgl.ldxy.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwgl.ldxy.edu.cn/jwglxt/student/xskb"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取陇东学院课表（青果教务）...');

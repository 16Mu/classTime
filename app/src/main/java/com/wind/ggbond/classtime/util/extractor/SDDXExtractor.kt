 ...
 * 山东大学课表提取器
 * 系统：强智教务系统-iframe版本
 * 参考：temp_aishedule/强智教务/iframe强智/山东大学/provider.js
 */
@Singleton
class SDDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "sddx"
    override val schoolName = "山东大学"
    override val systemType = "qiangzhi"

    override val aliases = listOf("山东大学", "山大")
    override val supportedUrls = listOf("bkjws.sdu.edu.cn")
    
    companion object {
        private const val TAG = "SDDXExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return (url.contains("/jsxsd/xskb/xskb_list.do") || url.contains("sdu.edu.cn")) &&
               (html.contains("kbtable") || html.contains("content_box"))
    }
    
    override fun getLoginUrl(): String = "http://bkjws.sdu.edu.cn/"
    override fun getScheduleUrl(): String = "http://bkjws.sdu.edu.cn/jsxsd/xskb/xskb_list.do"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取山东大学课表（强智iframe）...');

 ...
 * 盐城师范学院课表提取器
 * 系统：新正方教务系统
 * 参考：temp_aishedule/正方教务/新正方教务/盐城师范学院/provider.js
 */
@Singleton
class YCSFXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "ycsfxy"
    override val schoolName = "盐城师范学院"
    override val systemType = "zfsoft"

    override val aliases = listOf("盐城师范学院")
    override val supportedUrls = listOf("jwgl.yctc.edu.cn")
    
    companion object {
        private const val TAG = "YCSFXYExtractor"
    }
            ...
            (function() {
                try {
                    ...
                    try {
                        ...
                        if (ylkbTable) {
                            console.log('找到预览课表表格，使用HTML模式');
                            // HTML模式需要通过JavaScript解析，这里我们优先使用JSON API模式
                            throw new Error('使用JSON API模式');
                        }
                    } catch (e) {
                    ...
                    // 获取课表查询页面
                    var response = await fetch('/jwglxt/kbcx/xskbcx_cxXskbcxIndex.html?gnmkdm=' + id, {
                        method: 'GET'
                    });
                    ...
                    var response2 = await fetch('/jwglxt/kbcx/xskbcx_cxXsKb.html', {
                        method: 'POST',
                        body: formData,
                        headers: {
    ...
    override fun parseCourses(jsonData: String): List<ParsedCourse> {
        ...
        try {
            AppLogger.d(TAG, "开始解析盐城师范学院课程数据...")

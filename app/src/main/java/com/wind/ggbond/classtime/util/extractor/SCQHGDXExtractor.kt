 ...
 * 四川轻化工大学课表提取器
 * 系统：新正方教务系统
 * 参考：temp_aishedule/正方教务/新正方教务/四川轻化工大学/provider.js
 */
@Singleton
class SCQHGDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "scqhgdx"
    override val schoolName = "四川轻化工大学"
    override val systemType = "zfsoft"

    override val aliases = listOf("四川轻化工大学")
    override val supportedUrls = listOf("jwgl.suse.edu.cn")
    
    companion object {
        private const val TAG = "SCQHGDXExtractor"
    }
                ...
                try {
                    ...
                    try {
                        ...
                        if (forms) {
                            ...
                            var response = await fetch('/kbcx/xskbcxMobile_cxXsKb.html', {
                                method: 'POST',
                                body: formData,
                                headers: {
                        ...
                        var formData = 'xnm=' + xnm + '&xqm=' + xqm;
                        var response2 = await fetch('/kbcx/xskbcxMobile_cxXsKb.html', {
                            method: 'POST',
                            body: formData,
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded'
    ...
    override fun parseCourses(jsonData: String): List<ParsedCourse> {
        ...
        try {
            AppLogger.d(TAG, "开始解析四川轻化工大学课程数据...")

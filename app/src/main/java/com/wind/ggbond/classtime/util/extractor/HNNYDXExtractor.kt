...
@Singleton
class HNNYDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "hnnydx"
    override val schoolName = "河南农业大学"
    override val systemType = "kingosoft"

    override val aliases = listOf("河南农业大学", "河南农大")
    override val supportedUrls = listOf("jwgl.henau.edu.cn")
    
    companion object {
        private const val TAG = "HNNYDXExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("henau.edu.cn") && (url.contains("xskb") || html.contains("frmDesk"))
    }
    
    override fun getLoginUrl(): String = "http://jwgl.henau.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwgl.henau.edu.cn/jwglxt/student/xskb"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取河南农业大学课表（青果教务）...');

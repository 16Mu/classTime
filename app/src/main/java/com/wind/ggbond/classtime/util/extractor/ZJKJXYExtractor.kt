 ...
 * 湛江科技学院课表提取器
 * 系统：新正方教务系统
 * 参考：temp_aishedule/正方教务/新正方教务/湛江科技学院/provider.js
 */
@Singleton
class ZJKJXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "zjkjxy"
    override val schoolName = "湛江科技学院"
    override val systemType = "zfsoft"

    override val aliases = listOf("湛江科技学院")
    override val supportedUrls = listOf("jwgl.zjkjxy.edu.cn")
    
    companion object {
        private const val TAG = "ZJKJXYExtractor"
    }
    ...
    override fun getLoginUrl(): String {
        return "http://jwgl.zjkjxy.edu.cn/"
    }
            ...
            (function() {
                try {
                    ...
                    if (currentUrl.search("bjkbdy_cxBjkbdyIndex") != -1) {
                        var tableDom = document.getElementById("ylkbTable");
                        if (!tableDom) {
                            return JSON.stringify({courses: [], error: '请点击课程名称，打开课程表'});
                        }
                        
                        var form = document.getElementById("ajaxForm");
                        if (!form) {
                            return JSON.stringify({courses: [], error: '未找到表单'});
                        }
                        
                        xnm = form.xnm.value;
                        xqm = form.xqm.value;
                        // HTML模式暂不支持，优先使用JSON模式
                        return JSON.stringify({courses: [], error: '请使用个人课表查询页面'});
                    } 
                    // 检查是否在学生课表查询页面（JSON模式）
                        ...
                        var response = await fetch('/kbcx/xskbcx_cxXsgrkb.html', {
                            method: 'POST',
                            body: formData,
                            headers: {
                    ...
                    // 通过导航菜单查找
                        ...
                        var formData = 'xnm=' + xnm + '&xqm=' + xqm;
                        var response2 = await fetch('/kbcx/xskbcx_cxXsgrkb.html', {
                            method: 'POST',
                            body: formData,
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded'
    ...
    override fun parseCourses(jsonData: String): List<ParsedCourse> {
        ...
        try {
            AppLogger.d(TAG, "开始解析湛江科技学院课程数据...")

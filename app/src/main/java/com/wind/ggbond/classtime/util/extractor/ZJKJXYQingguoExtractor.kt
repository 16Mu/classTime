...
@Singleton
class ZJKJXYQingguoExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "zjkjxy_qingguo"
    override val schoolName = "湛江科技学院(青果)"
    override val systemType = "kingosoft"

    override val aliases = listOf("湛江科技学院(青果")
    override val supportedUrls = listOf("jwgl.zjkjedu.cn")
    
    companion object {
        private const val TAG = "ZJKJXYQingguoExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("zjkjedu.cn") && (url.contains("xskb") || html.contains("frmDesk"))
    }
    
    override fun getLoginUrl(): String = "http://jwgl.zjkjedu.cn/"
    override fun getScheduleUrl(): String = "http://jwgl.zjkjedu.cn/jwglxt/student/xskb"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取湛江科技学院课表（青果教务）...');

 ...
 * 广东农工商课表提取器
 * 系统：强智教务系统-iframe版本
 * 参考：temp_aishedule/强智教务/iframe强智/广东农工商/provider.js
 */
@Singleton
class GDNGSExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "gdngs"
    override val schoolName = "广东农工商职业技术学院"
    override val systemType = "qiangzhi"

    override val aliases = listOf("广东农工商职业技术学院", "广东农工商")
    override val supportedUrls = listOf("jwmis.gdaib.edu.cn")
    
    companion object {
        private const val TAG = "GDNGSExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return (url.contains("/jsxsd/xskb/xskb_list.do") || url.contains("gdaib.edu.cn")) &&
               (html.contains("kbtable") || html.contains("content_box"))
    }
    
    override fun getLoginUrl(): String = "http://jwmis.gdaib.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwmis.gdaib.edu.cn/jsxsd/xskb/xskb_list.do"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取广东农工商课表（强智iframe）...');

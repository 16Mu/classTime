 ...
 * 湖南软件职业技术学院课表提取器
 * 系统：强智教务系统-手机端
 * 参考：temp_aishedule/强智教务/手机端/湖南软件职业技术学院/provider.js
 */
@Singleton
class HNRJZYJSXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "hnrjzyjsxy"
    override val schoolName = "湖南软件职业技术学院"
    override val systemType = "qiangzhi"

    override val aliases = listOf("湖南软件职业技术学院", "湖南软件")
    override val supportedUrls = listOf("hnrjzyxyhd")
    
    companion object {
        private const val TAG = "HNRJZYJSXYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("hnrjzyxyhd") && 
               (url.contains("curriculum") || html.contains("sessionStorage"))
    }
    
    override fun getLoginUrl(): String = "http://222.243.161.213:81/"
    override fun getScheduleUrl(): String = "http://222.243.161.213:81/hnrjzyxyhd/student/curriculum"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取湖南软件职业技术学院课表（手机端）...');

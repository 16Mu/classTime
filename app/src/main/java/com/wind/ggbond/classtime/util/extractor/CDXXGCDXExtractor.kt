...
@Singleton
class CDXXGCDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    override val schoolId = "cdxxgcdx"
    override val schoolName = "成都信息工程大学"
    override val systemType = "shuwei"

    override val aliases = listOf("成都信息工程大学", "成信大")
    override val supportedUrls = listOf("jwgl.cuit.edu.cn")
    companion object { private const val TAG = "CDXXGCDXExtractor" }
    ...
    override fun getLoginUrl() = "http://jwgl.cuit.edu.cn/"
    override fun getScheduleUrl() = "http://jwgl.cuit.edu.cn/eams/courseTableForStd.action"
    
    override fun generateExtractionScript() = """
        (function() {
            try {
                console.log('🔍 开始提取成都信息工程大学课表（树维系统-版本一）...');
                var courses = [];

...
@Singleton
class XAHKXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    override val schoolId = "xahkxy"
    override val schoolName = "西安航空学院"
    override val systemType = "shuwei"

    override val aliases = listOf("西安航空学院", "西航")
    override val supportedUrls = listOf("jiaowu.xaau.edu.cn")
    companion object { private const val TAG = "XAHKXYExtractor" }
    ...
    override fun getLoginUrl() = "http://jiaowu.xaau.edu.cn/"
    override fun getScheduleUrl() = "http://jiaowu.xaau.edu.cn/eams/courseTableForStd.action"
    
    override fun generateExtractionScript() = """
        (function() {
            try {
                console.log('🔍 开始提取西安航空学院课表（树维系统-版本一）...');
                var courses = [];

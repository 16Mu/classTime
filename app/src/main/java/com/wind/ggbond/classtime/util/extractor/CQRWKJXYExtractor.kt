 ...
 * 重庆人文科技学院课表提取器
 * 系统：强智教务系统-iframe版本
 * 参考：temp_aishedule/强智教务/iframe强智/重庆人文科技学院/provider.js
 */
@Singleton
class CQRWKJXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "cqrwkjxy"
    override val schoolName = "重庆人文科技学院"
    override val systemType = "qiangzhi"

    override val aliases = listOf("重庆人文科技学院", "重庆人文")
    override val supportedUrls = listOf("jwgl.cqrk.edu.cn")
    
    companion object {
        private const val TAG = "CQRWKJXYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return (url.contains("/jsxsd/xskb/xskb_list.do") || url.contains("cqrk.edu.cn")) &&
               (html.contains("kbtable") || html.contains("content_box"))
    }
    
    override fun getLoginUrl(): String = "http://jwgl.cqrk.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwgl.cqrk.edu.cn/jsxsd/xskb/xskb_list.do"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取重庆人文科技学院课表（强智iframe）...');
                    ...
                    try {
                        if (window.frames && window.frames["Frame1"] && window.frames["Frame1"].document) {
                            ...
                            if (frameDoc.location.pathname.indexOf('/jsxsd/xskb/xskb_list.do') !== -1) {

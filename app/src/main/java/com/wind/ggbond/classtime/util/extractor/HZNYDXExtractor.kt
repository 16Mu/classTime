 ...
 * 华中农业大学课表提取器
 * 系统：新正方教务系统（HTML解析模式）
 * 参考：temp_aishedule/正方教务/新正方教务/华中农业大学/parser.js
 */
@Singleton
class HZNYDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "hznydx"
    override val schoolName = "华中农业大学"
    override val systemType = "zfsoft"

    override val aliases = listOf("华中农业大学")
    override val supportedUrls = listOf("jwgl.hzau.edu.cn")
    
    companion object {
        private const val TAG = "HZNYDXExtractor"
    }
                        ...
                        for (var i = 2; i < trs.length; i++) {
                            ...
                            tdWraps.forEach(function(tdWrap) {
                                ...
                                divs.forEach(function(div) {
                                    ...
                                    // 检查最后一个字符，如果不是中文或英文数字则移除
                                    if (course.courseName.length > 0) {
                                        var lastChar = course.courseName.slice(-1);
                                        if (!/[\u4E00-\u9FA5]|[a-zA-Z0-9]/.test(lastChar)) {
    ...
    override fun parseCourses(jsonData: String): List<ParsedCourse> {
        ...
        try {
            AppLogger.d(TAG, "开始解析华中农业大学课程数据...")

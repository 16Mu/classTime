...
@Singleton
class XAFYXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "xafyxy"
    override val schoolName = "西安翻译学院"
    override val systemType = "kingosoft"

    override val aliases = listOf("西安翻译学院", "西翻")
    override val supportedUrls = listOf("jwgl.xafy.edu.cn")
    
    companion object {
        private const val TAG = "XAFYXYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("xafy.edu.cn") && (url.contains("xskb") || html.contains("frmDesk"))
    }
    
    override fun getLoginUrl(): String = "http://jwgl.xafy.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwgl.xafy.edu.cn/jwglxt/student/xskb"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取西安翻译学院课表（青果教务）...');

 ...
 * 太原科技大学课表提取器
 * 系统：新正方教务系统（HTML解析模式）
 * 参考：temp_aishedule/正方教务/新正方教务/太原科技大学/Provider.js
 */
@Singleton
class TYKJDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "tykjdx"
    override val schoolName = "太原科技大学"
    override val systemType = "zfsoft"

    override val aliases = listOf("太原科技大学")
    override val supportedUrls = listOf("jwgl.tyust.edu.cn")
    
    companion object {
        private const val TAG = "TYKJDXExtractor"
    }
        ...
        return """
            (function() {
                        ...
                        for (var i = 2; i < trs.length; i++) {
                            ...
                            tdWraps.forEach(function(tdWrap) {
                                ...
                                divs.forEach(function(div) {
                                    ...
                                    // 截取前50字节
                                    course.courseName = cutOutStringByteLength(course.courseName, 50);
                                    ...
                                    pTags.forEach(function(p) {
                                        ...
                                        switch(title) {
                                            case '节/周':
                                                ...
                                                break;
                                            case '上课地点':
                                                course.classroom = text.replace(/晋城校区|主校区|南校区|南社校区/g, '').trim();
                                                course.classroom = cutOutStringByteLength(course.classroom, 50);
                                                break;
                                            case '教师':
                                                course.teacher = text.replace(/\\([^)]*\\)/g, '').trim();
                                                course.teacher = cutOutStringByteLength(course.teacher, 50);
                                                break;
                                        }
                                    });
                                ...
                                fontTags.forEach(function(font) {
                                    ...
                                    switch(className) {
                                        ...
                                        case 'glyphicon glyphicon-map-marker':
                                            course.classroom = text.replace(/校区：|校本部|晋城校区|主校区|南校区|南社校区|上课地点：/g, '').trim();
                                            course.classroom = cutOutStringByteLength(course.classroom, 50);
                                            break;
                ...
                function getWeeks(str) {
                    ...
                    var reweek = [];
                    week1.filter(function(s) { return s && s.trim(); }).forEach(function(v) {
                        if (v.substring(v.length - 1) === '双') {
                            v.substring(0, v.length - 1).split(',').forEach(function(w) {
                                var tt = w.split('-').filter(function(s) { return s && s.trim(); });
                                for (var z = Number(tt[0]); z <= tt[tt.length - 1]; z++) {
    ...
    override fun parseCourses(jsonData: String): List<ParsedCourse> {
        ...
        try {
            AppLogger.d(TAG, "开始解析太原科技大学课程数据...")

 ...
 * 武汉信息传播职业技术学院课表提取器
 * 系统：强智教务系统-iframe版本
 * 参考：temp_aishedule/强智教务/iframe强智/武汉信息传播职业技术学院/provider.js
 */
@Singleton
class WHXXCBZYJSXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "whxxcbzyjsxy"
    override val schoolName = "武汉信息传播职业技术学院"
    override val systemType = "qiangzhi"

    override val aliases = listOf("武汉信息传播职业技术学院", "武信")
    override val supportedUrls = listOf("jwgl.whxxcb.com")
    
    companion object {
        private const val TAG = "WHXXCBZYJSXYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return (url.contains("/xskb/xskb_list.do") || url.contains("whxxcb.com")) &&
               (html.contains("kbtable") || html.contains("content_box"))
    }
    
    override fun getLoginUrl(): String = "http://jwgl.whxxcb.com/"
    override fun getScheduleUrl(): String = "http://jwgl.whxxcb.com/xskb/xskb_list.do"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取武汉信息传播职业技术学院课表（强智iframe）...');
                    ...
                    // 优先从Frame1获取
                    try {
                        if (window.frames && window.frames["Frame1"] && window.frames["Frame1"].document) {
                            let frameDoc = window.frames["Frame1"].document;
                            let kbtable = frameDoc.getElementById('kbtable');
                            if (kbtable) {
                                html = kbtable.outerHTML;
                                found = true;
                            } else {
                                let contentBox = frameDoc.getElementsByClassName('content_box')[0];
                                if (contentBox) {
                        ...
                        console.log('无法访问Frame1:', e);
                    }
                    
                    // 如果Frame1失败，尝试标准iframe
                    if (!found) {
                        let iframes = document.getElementsByTagName('iframe');
                        for (let i = 0; i < iframes.length; i++) {
                            let iframe = iframes[i];
                            if (iframe.src && iframe.src.indexOf('/xskb/xskb_list.do') !== -1) {
                                try {

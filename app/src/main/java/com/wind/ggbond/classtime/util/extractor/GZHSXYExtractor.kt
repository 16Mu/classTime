 ...
 * 广州华商学院课表提取器
 * 系统：强智教务系统-iframe版本
 * 参考：temp_aishedule/强智教务/iframe强智/广州华商学院/provider.js
 */
@Singleton
class GZHSXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "gzhsxy"
    override val schoolName = "广州华商学院"
    override val systemType = "qiangzhi"

    override val aliases = listOf("广州华商学院", "华商学院")
    override val supportedUrls = listOf("jwc.gdhsc.edu.cn")
    
    companion object {
        private const val TAG = "GZHSXYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return (url.contains("/jsxsd/xskb/xskb_list.do") || url.contains("gdhsc.edu.cn")) &&
               (html.contains("kbtable") || html.contains("content_box"))
    }
    
    override fun getLoginUrl(): String = "http://jwc.gdhsc.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwc.gdhsc.edu.cn/jsxsd/xskb/xskb_list.do"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取广州华商学院课表（强智iframe）...');

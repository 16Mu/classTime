 ...
 * 燕山大学课表提取器
 * 系统：金智教务系统
 * 参考：temp_aishedule/金智教务/燕山大学/provider.js
 */
@Singleton
class YSDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "ysdx"
    override val schoolName = "燕山大学"
    override val systemType = "jinzhi"

    override val aliases = listOf("燕山大学")
    override val supportedUrls = listOf("jwgl.ysu.edu.cn")
    
    companion object {
        private const val TAG = "YSDXExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("wdkb") ||
               url.contains("xskcb") ||
               url.contains("cxxszhxqkb") ||
               html.contains("kcb_container") ||
               html.contains("dqxnxq")
    }
    
    override fun getLoginUrl(): String {
        return "http://jwgl.ysu.edu.cn/"
    }
    
    override fun getScheduleUrl(): String {
        return "/jwapp/sys/wdkb/modules/xskcb/cxxszhxqkb.do"
    }
        ...
        return """
            (function() {
                try {
                    ...
                    // 获取课表数据（燕山大学使用cxxszhxqkb.do）
                    console.log('获取课表数据，XNXQDM: ' + xnxq);
                    var formData = "XNXQDM=" + xnxq;
                    var response2 = await fetch("/jwapp/sys/wdkb/modules/xskcb/cxxszhxqkb.do", {
                        method: 'POST',
                        body: formData,
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
                    ...
                    if (!courseData.datas || !courseData.datas.cxxszhxqkb || !courseData.datas.cxxszhxqkb.rows) {
                        return JSON.stringify({courses: [], error: '未获取到课表数据'});
                    }
                    
                    var coursesList = courseData.datas.cxxszhxqkb.rows;
                    var courses = [];

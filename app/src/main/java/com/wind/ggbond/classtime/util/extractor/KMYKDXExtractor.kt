 ...
 * 昆明医科大学课表提取器
 * 系统：青果教务系统
 * 参考：temp_aishedule/青果教务/昆明医科大学/provider.js
 */
@Singleton
class KMYKDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    ...
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("kmmu.edu.cn") && 
               (url.contains("xskb") || html.contains("frmDesk"))
    }
    
    override fun getLoginUrl(): String = "http://jwxt.kmmu.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwxt.kmmu.edu.cn/jwglxt/student/xskb"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取昆明医科大学课表（青果教务）...');

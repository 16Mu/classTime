 ...
 * 北京财贸职业学院课表提取器-手机端版本
 * 系统：强智教务系统-手机端
 * 参考：temp_aishedule/强智教务/手机端/北京财贸职业学院/provider.js
 */
@Singleton
class BJCMZYXYMobileExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "bjcmzyxy_mobile"
    override val schoolName = "北京财贸职业学院(手机端)"
    override val systemType = "qiangzhi"

    override val aliases = listOf("北京财贸职业学院(手机端")
    override val supportedUrls = listOf("/bjcmzyxy/")
    
    companion object {
        private const val TAG = "BJCMZYXYMobileExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("bjczy.edu.cn") && 
               (url.contains("/bjcmzyxy/") || html.contains("sessionStorage"))
    }
    
    override fun getLoginUrl(): String = "http://jwgl.bjczy.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwgl.bjczy.edu.cn/bjcmzyxy/student/curriculum"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取北京财贸职业学院课表（手机端）...');

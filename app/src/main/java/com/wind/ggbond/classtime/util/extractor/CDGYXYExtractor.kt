 ...
 * 成都工业学院课表提取器
 * 系统：强智教务系统-iframe版本
 * 参考：temp_aishedule/强智教务/iframe强智/成都工业学院/provider.js
 */
@Singleton
class CDGYXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "cdgyxy"
    override val schoolName = "成都工业学院"
    override val systemType = "qiangzhi"

    override val aliases = listOf("成都工业学院", "成工院")
    override val supportedUrls = listOf("jwglxt.cdtu.edu.cn")
    
    companion object {
        private const val TAG = "CDGYXYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return (url.contains("/jsxsd/xskb/xskb_list.do") || url.contains("cdtu.edu.cn")) &&
               (html.contains("kbtable") || html.contains("content_box"))
    }
    
    override fun getLoginUrl(): String = "http://jwglxt.cdtu.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwglxt.cdtu.edu.cn/jsxsd/xskb/xskb_list.do"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取成都工业学院课表（强智iframe）...');

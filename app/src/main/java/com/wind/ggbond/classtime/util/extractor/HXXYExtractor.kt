...
@Singleton
class HXXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "hxxy"
    override val schoolName = "河西学院"
    override val systemType = "kingosoft"

    override val aliases = listOf("河西学院", "河西")
    override val supportedUrls = listOf("jwxt.hxu.edu.cn")
    
    companion object {
        private const val TAG = "HXXYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("hxu.edu.cn") && (url.contains("xskb") || html.contains("frmDesk"))
    }
    
    override fun getLoginUrl(): String = "http://jwxt.hxu.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwxt.hxu.edu.cn/jwglxt/student/xskb"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取河西学院课表（青果教务）...');

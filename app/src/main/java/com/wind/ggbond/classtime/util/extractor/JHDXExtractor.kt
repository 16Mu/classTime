...
@Singleton
class JHDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "jhdx"
    override val schoolName = "江汉大学"
    override val systemType = "kingosoft"

    override val aliases = listOf("江汉大学", "江大")
    override val supportedUrls = listOf("jwgl.jhun.edu.cn")
    
    companion object {
        private const val TAG = "JHDXExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("jhun.edu.cn") && (url.contains("xskb") || html.contains("frmDesk"))
    }
    
    override fun getLoginUrl(): String = "http://jwgl.jhun.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwgl.jhun.edu.cn/jwglxt/student/xskb"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取江汉大学课表（青果教务）...');

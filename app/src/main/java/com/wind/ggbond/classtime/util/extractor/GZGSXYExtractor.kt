 ...
 */
@Singleton
class GZGSXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "gzgsxy"
    override val schoolName = "广州工商学院"
    override val systemType = "shuwei"

    override val aliases = listOf("广州工商学院", "广工商")
    override val supportedUrls = listOf("eams.gzgs.edu.cn")
    
    companion object {
        private const val TAG = "GZGSXYExtractor"
    }
    ...
    override fun getLoginUrl(): String = "http://eams.gzgs.edu.cn/"
    override fun getScheduleUrl(): String = "http://eams.gzgs.edu.cn/eams/courseTableForStd.action"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取广州工商学院课表（树维系统-版本一）...');
                    var courses = [];
    ...
    override fun parseCourses(jsonData: String): List<ParsedCourse> {
        ...
        try {
            AppLogger.d(TAG, "开始解析广州工商学院课程数据...")

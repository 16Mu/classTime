 ...
 * 山东女子学院课表提取器
 * 系统：青果教务系统
 * 参考：temp_aishedule/青果教务/山东女子学院/provider.js
 */
@Singleton
class SDNZXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "sdnzxy"
    override val schoolName = "山东女子学院"
    override val systemType = "kingosoft"

    override val aliases = listOf("山东女子学院", "山东女院")
    override val supportedUrls = listOf("jwxt.sdwu.edu.cn")
    
    companion object {
        private const val TAG = "SDNZXYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("sdwu.edu.cn") && 
               (url.contains("xskb") || html.contains("frmDesk"))
    }
    
    override fun getLoginUrl(): String = "http://jwxt.sdwu.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwxt.sdwu.edu.cn/sdnzjw/frame/desk"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取山东女子学院课表（青果教务）...');
                    ...
                    try {
                        if (window.frames && window.frames['frmDesk']) {
                            ...
                            if (frmDesk.document.getElementById('xnxq') && !frmDesk.frames['frame_1']) {
                                let lessonContent = frmDesk.document.getElementById('lessonSchedule-content');
                                if (lessonContent) {
                                    html = lessonContent.outerHTML;
                                    bz = 'index';
                                }
                            } else if (frmDesk.frames['frame_1'] && frmDesk.frames['frame_1'].contentDocument.getElementById('cxfs_ewb')) {

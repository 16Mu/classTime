...
@Singleton
class ZZSDJMGLXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "zzsdjmglxy"
    override val schoolName = "郑州升达经贸管理学院"
    override val systemType = "kingosoft"

    override val aliases = listOf("郑州升达经贸管理学院", "郑州升达")
    override val supportedUrls = listOf("jwgl.shengda.edu.cn")
    
    companion object {
        private const val TAG = "ZZSDJMGLXYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("shengda.edu.cn") && (url.contains("xskb") || html.contains("frmDesk"))
    }
    
    override fun getLoginUrl(): String = "http://jwgl.shengda.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwgl.shengda.edu.cn/jwglxt/student/xskb"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取郑州升达经贸管理学院课表（青果教务）...');

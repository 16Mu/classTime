 ...
 * 华北水利水电大学课表提取器-旧版
 * 系统：青果教务系统
 * 参考：temp_aishedule/青果教务/华北水利水电_旧版/provider.js
 */
@Singleton
class HBSLSDOldExtractor @Inject constructor() : SchoolScheduleExtractor {
    ...
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("ncwu.edu.cn") && 
               (url.contains("xskb") || url.contains("wsxk.xskcb") || html.contains("frmbody"))
    }
    ...
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取华北水利水电大学课表（青果教务-旧版）...');
                    ...
                    try {
                        // 从frmbody.frmDesk获取（旧版特有）
                        if (window.frames && window.frames['frmbody'] && window.frames['frmbody'].frames['frmDesk']) {
                            let frmDesk = window.frames['frmbody'].frames['frmDesk'];
                            
                            // index模式
                            if (frmDesk.document.getElementById('xnxq') && !frmDesk.frames['frmReport']) {
                                let lessonContent = frmDesk.document.getElementById('lessonSchedule-content');
                                if (lessonContent) {
                                    html = lessonContent.outerHTML;
                                    bz = 'index';
                                }
                            } else if (frmDesk.document.getElementById('frame_1')) {
                                let frame1 = frmDesk.document.getElementById('frame_1').contentWindow;
                                let frame1Doc = frame1.document;
                                ...
                                if (frame1.frames['frmReport']) {
                                    let frmReportDoc = frame1.frames['frmReport'].document;

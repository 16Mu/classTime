 ...
 */
@Singleton
class CQUSTExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "cqust"
    override val schoolName = "重庆科技学院"
    override val systemType = "shuwei"

    override val aliases = listOf("重庆科技学院")
    override val supportedUrls = listOf("web.cqust.edu.cn")
    
    companion object {
        private const val TAG = "CQUSTExtractor"
    }
    ...
    override fun getLoginUrl(): String = "http://web.cqust.edu.cn/"
    override fun getScheduleUrl(): String = "http://222.179.135.21/eams/courseTableForStd.action"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取${schoolName}课表（树维系统）...');
                    var courses = [];

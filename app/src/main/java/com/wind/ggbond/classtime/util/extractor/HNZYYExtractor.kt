 ...
 * 湖南中医药大学课表提取器
 * 系统：强智教务系统-iframe版本
 * 参考：temp_aishedule/强智教务/iframe强智/湖南中医药/Provider.js
 */
@Singleton
class HNZYYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "hnzyy"
    override val schoolName = "湖南中医药大学"
    override val systemType = "qiangzhi"

    override val aliases = listOf("湖南中医药大学", "湖南中医药")
    override val supportedUrls = listOf("jwgl.hnucm.edu.cn")
    
    companion object {
        private const val TAG = "HNZYYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return (url.contains("/jsxsd/xskb/xskb_list.do") || url.contains("hnucm.edu.cn")) &&
               (html.contains("kbtable") || html.contains("content_box"))
    }
    
    override fun getLoginUrl(): String = "http://jwgl.hnucm.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwgl.hnucm.edu.cn/jsxsd/xskb/xskb_list.do"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取湖南中医药大学课表（强智iframe）...');
                    ...
                    try {
                        if (window.frames && window.frames["Frame1"] && window.frames["Frame1"].document) {
                            ...
                            if (frameDoc.location.pathname === '/jsxsd/xskb/xskb_list.do') {

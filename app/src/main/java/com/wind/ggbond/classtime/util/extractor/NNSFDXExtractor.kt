 ...
 * 南宁师范大学课表提取器
 * 系统：强智教务系统-iframe版本
 * 参考：temp_aishedule/强智教务/iframe强智/南宁师范大学/provider.js
 */
@Singleton
class NNSFDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "nnsfdx"
    override val schoolName = "南宁师范大学"
    override val systemType = "qiangzhi"

    override val aliases = listOf("南宁师范大学", "南宁师大")
    override val supportedUrls = listOf("jwc.nnnu.edu.cn")
    
    companion object {
        private const val TAG = "NNSFDXExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return (url.contains("/jsxsd/xskb/xskb_list.do") || url.contains("nnnu.edu.cn")) &&
               (html.contains("kbtable") || html.contains("content_box"))
    }
    
    override fun getLoginUrl(): String = "http://jwc.nnnu.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwc.nnnu.edu.cn/jsxsd/xskb/xskb_list.do"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取南宁师范大学课表（强智iframe）...');
                        ...
                        cells.forEach((cell, day) => {
                            ...
                            kbcontents.forEach((kbcontent) => {
                                ...
                                courseBlocks.forEach((courseBlock) => {
                                    let parts = courseBlock.split(/<br>/i);

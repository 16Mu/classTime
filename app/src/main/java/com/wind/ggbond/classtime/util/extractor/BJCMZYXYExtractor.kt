 ...
 * 北京财贸职业学院课表提取器
 * 系统：强智教务系统-iframe版本
 * 参考：temp_aishedule/强智教务/iframe强智/北京财贸职业学院/provider.js
 */
@Singleton
class BJCMZYXYExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "bjcmzyxy"
    override val schoolName = "北京财贸职业学院"
    override val systemType = "qiangzhi"

    override val aliases = listOf("北京财贸职业学院", "北京财贸")
    override val supportedUrls = listOf("jwgl.bjczy.edu.cn")
    
    companion object {
        private const val TAG = "BJCMZYXYExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return (url.contains("/jsxsd/xskb/xskb_list.do") || url.contains("jwgl.bjczy.edu.cn")) &&
               (html.contains("kbtable") || html.contains("content_box"))
    }
    
    override fun getLoginUrl(): String = "http://jwgl.bjczy.edu.cn/"
    override fun getScheduleUrl(): String = "http://jwgl.bjczy.edu.cn/jsxsd/xskb/xskb_list.do"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取北京财贸职业学院课表（强智iframe）...');

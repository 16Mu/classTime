 ...
 * 新疆大学课表提取器
 * 系统：青果教务系统
 * 参考：temp_aishedule/青果教务/新疆大学/provider.js
 */
@Singleton
class XJDXExtractor @Inject constructor() : SchoolScheduleExtractor {
    
    override val schoolId = "xjdx"
    override val schoolName = "新疆大学"
    override val systemType = "kingosoft"

    override val aliases = listOf("新疆大学", "新大")
    override val supportedUrls = listOf("portal.xju.edu.cn")
    
    companion object {
        private const val TAG = "XJDXExtractor"
    }
    
    override fun isSchedulePage(html: String, url: String): Boolean {
        return url.contains("xju.edu.cn") && 
               (url.contains("xskb") || html.contains("frmDesk"))
    }
    
    override fun getLoginUrl(): String = "http://portal.xju.edu.cn/"
    override fun getScheduleUrl(): String = "http://portal.xju.edu.cn/jwglxt/student/xskb"
    
    override fun generateExtractionScript(): String {
        return """
            (function() {
                try {
                    console.log('🔍 开始提取新疆大学课表（青果教务）...');
